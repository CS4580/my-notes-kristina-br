...
def _get_weighted_jaccard_similarty_dict(df):
    ...
    for movie in selections_df:
        for genre in movie['genres'].split(';'):
            if genre in genres_weighted_dictionary:
                genres_weighted_dictionary[genre] += 1  # make new entry
            else:
                genres_weighted_dictionary[genre] = 1
            genres_weighted_dictionary['total'] += 1

    return genres_weighted_dectionary


def jaccard_similarity_weighted(df: pd.DataFrame, comparator_genre: str):
    weighted_dictionary = _get_weighted_jaccard_similarty_dict(df)
    numerator = 0
    # comparing against the rest of the records
...
def knn_analysis_driver(data_df, base_case, comparison_type, metric_func, sorted_value='metric'):
    ...
    # WIP: Create df of filter data -- NOTE that weighted has slightly diff stuff
